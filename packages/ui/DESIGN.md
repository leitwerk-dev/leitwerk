...
- **Process Inspector:** Replaces the process content region beside global navigation. A persistent identity and context-origin header leads into one scrollable evidence pane. Process sections are Overview, Workflow, Inputs & configuration, and Context map; execution sections are Trace, Context, and Configuration. Back retraces explicit investigation links. Show in chronicle reveals the selected execution or restores the original reading position and focus. The hidden, inert chronicle stays mounted to preserve drafts and disclosure state. See [Process inspector](surfaces/process-inspector.md).
