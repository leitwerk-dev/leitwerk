...
- Completed and live cards place Show reasoning immediately beside Turn details in the final footer, opening the existing reasoning overlay. Omit Show reasoning when no details exist. Live reasoning previews, open questions, answer forms, and live stop controls remain above that footer.
...
- The completed-card footer uses two top-aligned columns. Workspace prepared expands within the left column. Create issue and the detail links remain in the right column. Narrow cards with reasoning put their actions above preparation, keeping Show reasoning and Turn details together when actions wrap. Preparation expands without moving the links on desktop or mobile.
- Each item of a mapped LLM turn is a separate card and rail row. Its title appends the item label to the turn description, verbatim, and its metadata leads with Item N of M. While an item runs, the pending rail row names the next item. Distinct items never fold into Earlier updates; retries of one item group as Earlier attempts.
