...
<div class="process-reference">
  ...
  {:else if target.section === "overview"}
    ...
    {#if detail.launchConfiguration.projects.length}<section><h2>Recorded repositories and branches</h2><p class="note">Repository configuration does not prove a repository was changed.</p>{#each detail.launchConfiguration.projects as project (project.key)}<div class="repository"><h3>{project.key}</h3><p><code>{project.repoLocator}</code></p><dl><div><dt>Base branch</dt><dd>{project.baseBranch}</dd></div><div><dt>Work branch</dt><dd>{project.workBranch ?? "Not recorded"}</dd></div></dl>{#if project.externalUrl}<ExternalLink href={project.externalUrl} label="Open project" resourceType="project" />{/if}</div>{/each}</section>{/if}
    <section><h2>Deliverables and evidence</h2>{#if resources.length}<ul class="resource-list">{#each resources as resource (`${resource.turnRecordId}:${resource.id}`)}<li><ExternalLink href={resource.url} label={resource.label} resourceType="evidence" /><button class="text-link" onclick={() => onNavigate({scope:"execution", turnRecordId:resource.turnRecordId, section:"trace"})}>Recorded by execution</button></li>{/each}</ul>{:else}<p class="note">No deliverable links were recorded. Results and decisions remain in the chronicle.</p>{/if}</section>
  ...
  {:else if target.section === "context-map"}
    <section><h2>Context map</h2><p class="note">Conversation inheritance and explicitly supplied products. Unconnected executions can have unknown context; they are not assumed to be fresh.</p>
      <ConversationTreeDiagram tree={detail.instanceTree} railItems={[]} selectedTurnRecordId={target.turnRecordId} onSelectExecution={selectExecution} />
...
dl {display:grid; gap:var(--space-md);} dl > div {display:grid; grid-template-columns:minmax(120px,1fr) 2fr; gap:var(--space-md);} .fact-grid {grid-template-columns:repeat(2,minmax(0,1fr));} .fact-grid > div {display:block;} dt {font-size:var(--type-caption);} code {font-size:var(--type-caption); overflow-wrap:anywhere;}
.repository {display:grid; gap:var(--space-sm); padding:var(--space-sm) 0;} .ui-button {justify-self:start;}
...
.context-list li {padding:var(--space-sm) var(--space-md); border:1px solid var(--chronicle-border); border-radius:var(--radius-sm);} .context-list li.selected {background:var(--chronicle-accent-soft);}
.notice {padding:var(--space-md); background:var(--chronicle-panel-muted); border-radius:var(--radius-sm);}
@media(max-width:720px) {.fact-grid,.step-list {grid-template-columns:1fr;} dl > div {grid-template-columns:1fr; gap:var(--space-xs);} }
</style>
