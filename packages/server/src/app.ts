...
export async function createAppContext(opts: AppOptions = {}): Promise<AppContext> {
	...
	try {
		...
		async function reconcileStartup(assertOpen: () => void): Promise<void> {
			if (!supervisor) {
		...
		const processEngine = createProcessEngine({
			...
			mappedRuns: baseDeps.mappedRuns,
			transaction: baseDeps.transaction,
			broadcaster,
			config,
			toastTtlMs,
		...
		const ipcHandler = createIpcHandler(
			{
				...
				updateCredential: ({ providerId, expectedRevision, values }) => {
					...
					if (result.accepted && !lifecycle.isClosing()) {
						const work = refreshModelStatusAndApplyTransitions().catch((error: unknown) => {
							app.log.warn(
								{ error, providerId },
		...
		const lifecycle = createAppLifecycle({
			...
			authEnabled: authService.config.enabled,
			reconcile: reconcileStartup,
			startHooks,
			preCloseCleanup,
			closeDatabase: () => closeOwnedDatabase?.(),
		});
		finishCleanup = lifecycle.finishCleanup;
