...
export interface VolumeRef {
	/** @internal */
	instanceId: string;
	/** Runtime-specific identifier (host path, named volume, PVC name). @internal */
	id: string;
	/** Path inside the worker container where the volume is mounted. @internal */
	mountPath: string;
	/** Runtime-specific namespace/project for the volume (Kubernetes process namespace). @internal */
	namespace?: string;
}

/** @internal */
export interface ProcessVolumeRequirements {
	/** New Kubernetes PVC capacity. Existing volumes are unchanged; other runners ignore it. @internal */
	size?: string;
	/** Select storage compatible with a private Docker daemon. @internal */
	docker?: boolean;
}
...
/** Server-owned, credential-scoped relay used by isolated export helpers. @internal */
...
/** Purpose-specific read-only export seam. It never creates a worker lease. @internal */
export interface ProcessStateExporter {
	...
	/** Removes stale runner-specific export helpers after restart. @internal */
...
export interface ProcessVolume {
	/** Idempotently provisions durable storage for a process instance. @internal */
	ensure(
		instanceId: string,
		requirements?: ProcessVolumeRequirements,
		observer?: WorkerStartObserver,
	): Promise<VolumeRef>;
	/** Retention cleanup only. Stopping a worker never calls this. @internal */
	release(instanceId: string): Promise<void>;
	/** Permanently deletes every runner-managed resource owned by a deleted process. @internal */
...
export interface ResolvedWorkerImage {
	/** Fully qualified image reference, ideally digest-pinned. @internal */
	reference: string;
}
...
export interface WorkerUnitRef {
	...
	/** @internal */
	workerId: string;
	/** Runtime-specific handle id (container id, pod name). @internal */
	unitId: string;
	/** Runtime-specific namespace/project for the unit (Kubernetes process namespace). @internal */
	namespace?: string;
}
...
export interface WorkerExitInfo {
	/** Process exit code when the runtime reports one. @internal */
	exitCode: number | null;
	/** Termination signal when the runtime reports one. @internal */
	signal: string | null;
	/** True when the runtime killed the unit for exceeding memory limits. @internal */
	oomKilled?: boolean;
	/** Human-readable reason surfaced by the runtime, if any. @internal */
	reason?: string;
}

/** @internal */
export interface WorkerUnit extends WorkerUnitRef {
	/** Require successful runtime removal before the supervisor permits replacement. @internal */
	replacementHandoff?: "stop-before-replacement";
	/** Maps physical worker exit into the supervisor's existing failure paths. @internal */
...
interface StartWorkerInputBase {
	...
	/** @internal */
	serverEpoch: string;
	/** Runtime-profile selection output; MVP = single image. @internal */
	image: ResolvedWorkerImage;
	/** Server URL, ids, connect token, `PI_CODING_AGENT_DIR`, etc. @internal */
	env: Record<string, string>;
	/** Start the image's private Docker daemon before the worker. @internal */
	docker: boolean;
	/** @internal */
...
/** Start input for the best-effort local runner, which uses configured host storage. @internal */
...
/** Start input for isolated runners, whose durable process volume is mandatory. @internal */
...
export interface StopWorkerOptions {
	/** Grace period before forceful termination. The volume is always kept. @internal */
	graceMs: number;
}
...
export interface WorkerRunner<TStartInput extends StartWorkerInput = StartWorkerInput> {
	...
	/** Stops the unit but keeps the process volume for resume. @internal */
	stop(ref: WorkerUnitRef, opts: StopWorkerOptions): Promise<void>;
	/** Adoption scan: lists labelled worker units. @internal */
	list(): Promise<WorkerUnitDescriptor[]>;
	/** Reconnects to a live unit discovered by {@link WorkerRunner.list}. @internal */
