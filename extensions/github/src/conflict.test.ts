...
function fixture() {
	...
	const deps = {
		...
		externalSources: {
			listArmed: (kind: string) => (kind === GITHUB_PR_STATE_KIND ? [structuredClone(armed)] : []),
			fire,
			observe,
		},
	} as unknown as CoreServerSetupDeps;
	const integration = { client: () => client } as never;
	const provider = createGitHubProvider(deps, integration, { now: () => now });
	return {
		pr,
		armed,
		config,
...
describe("GitHub conflict polling", () => {
	...
	it("retries unknown or missing mergeability and ignores blocked and terminal PRs", async () => {
		const f = fixture();
		for (const value of [undefined, null, true]) {
			f.pr.mergeable = value;
			f.pr.mergeable_state = "blocked";
			await f.poll();
		}
	...
	it("leaves UI-mergeable PRs alone even when head and base differ", async () => {
		const f = fixture();
		f.pr.mergeable = true;
	...
	it("repairs a mergeable PR blocked by the behind merge state using the current base", async () => {
		const f = fixture();
		f.pr.mergeable = true;
	...
	it("retries unresolved behind metadata and branch refresh failures without firing", async () => {
		const f = fixture();
		f.pr.mergeable_state = "behind";
