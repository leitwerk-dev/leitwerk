...
import { resolveGitHubProjectBinding as resolve } from "./binding.js";
...
describe("github project binding", () => {
	it("uses each project's own profile without reading unrelated parameters", () => {
		for (const profile of ["first", "second"])
			expect(
				resolve(
					ctx({ github: { owner: "team", repo: profile, profile } }, { githubProfile: "wrong" }),
				),
			).toEqual({ owner: "team", repo: profile, profile });
		const context = ctx({ github: { owner: "team", repo: "repo", profile: "explicit" } });
	...
	it("retains stored legacy binding shapes", () => {
		expect(
			resolve(ctx({ github: { owner: "team", repo: "repo" } }, { githubProfile: "legacy" })),
		).toEqual({ owner: "team", repo: "repo", profile: "legacy" });
	});
	it("rejects missing and foreign projects", () => {
		const context = ctx({ github: { owner: "team", repo: "repo", profile: "one" } });
	...
	it.each([
		...
		expect(() => resolve(ctx({ github: binding }, { githubProfile: "legacy" }))).toThrow();
	});
});
