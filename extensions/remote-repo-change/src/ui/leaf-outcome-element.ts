...
/**
 * @deprecated Compatibility renderer for historical remote_repo_change_process leaf snapshots.
 * Removing this renderer is a breaking change for persisted snapshots whose rendererId is
 * @leitwerk-dev/remote-repo-change:remote_repo_change_process.leaf_outcome.
 */
interface LegacyRemoteRepoChangeLeafOutcomePayload {
	markdown?: unknown;
	summary?: unknown;
	result?: unknown;
	[key: string]: unknown;
...
const TAG_NAME = "o2-remote-repo-change-legacy-leaf-outcome";
...
function stringifyPayload(payload: LegacyRemoteRepoChangeLeafOutcomePayload | null): string {
	if (!payload) {
		return "{}";
	}
	try {
...
class LegacyRemoteRepoChangeLeafOutcomeElement extends LeafOutcomeCustomElement<LegacyRemoteRepoChangeLeafOutcomePayload> {
	protected renderContent(): void {
		...
		this.shadowRootRef.innerHTML = `
			...
			<article class="legacy-wrap" data-renderer="remote-repo-change-legacy-leaf-outcome">
				${bodyMarkup}
				${capturedMarkup}
			</article>
		`;
...
if (!customElements.get(TAG_NAME)) {
	customElements.define(TAG_NAME, LegacyRemoteRepoChangeLeafOutcomeElement);
}
