...
import { resolveWoodpeckerProjectBinding as resolve } from "./binding.js";
...
describe("woodpecker project binding", () => {
	it("uses each project's own profile without reading unrelated parameters", () => {
		for (const profile of ["first", "second"])
			expect(
				resolve(
					ctx(
						{ woodpecker: { owner: "team", repo: profile, profile } },
						{ woodpeckerProfile: "wrong" },
					),
				),
			).toEqual({ owner: "team", repo: profile, profile });
		const context = ctx({ woodpecker: { owner: "team", repo: "repo", profile: "explicit" } });
	...
	it("retains stored legacy binding shapes", () => {
		expect(
			resolve(ctx({ forgejo: { owner: "team", repo: "repo" } }, { woodpeckerProfile: "legacy" })),
		).toEqual({ owner: "team", repo: "repo", profile: "legacy" });
	});
	it("rejects missing and foreign projects", () => {
		const context = ctx({ woodpecker: { owner: "team", repo: "repo", profile: "one" } });
	...
	it.each([
		...
		expect(() => resolve(ctx({ woodpecker: binding }, { woodpeckerProfile: "legacy" }))).toThrow();
	});
});
