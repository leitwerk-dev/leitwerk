...
async function draft(f: Fixture, instructions = "Create a notebook issue.", kind = "turn_result") {
	...
	const request = {
		...
		payload: {
			...
			artifact:
				kind === "turn_result"
					? { kind, turnRecordId: turn.id }
					: { kind, leafEntryId: turn.resultPiEntryId },
...
async function decide(f: Fixture, id: string, action: string, feedback?: string) {
	...
	const response = await f.context.app.inject({
		method: "POST",
		url: `/api/processes/${id}/tool-approval-requests/${pending.id}`,
...
test("restarts with an approval, reconciles a lost ticket response, and retains one durable receipt", async () => {
	...
	try {
		...
		await approval(f, id);
		const injected = await f.context.app.inject({
			method: "POST",
			url: "/__local/lost-response",
		...
		expect(
			JSON.parse(readFileSync(path.join(f.root, "tickets.json"), "utf8")).tickets,
		).toHaveLength(1);
	} finally {
		await f.close();
	}
...
test("clarifies destinations, refines a draft through feedback, and declines without publishing", async () => {
	...
	try {
		...
		const answered = await f.context.app.inject({
			method: "POST",
...
test("launches from a retained leaf outcome and validates excerpt ownership", async () => {
	const f = await fixture();
	try {
		const parent = await f.launch("ticket");
		await f.wait(parent, "plan_decision");
