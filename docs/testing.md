...
- **Browser isolation:** Each Playwright invocation selects ephemeral API and UI ports, inherited by its workers through `LEITWERK_BROWSER_API_PORT` and `LEITWERK_BROWSER_UI_PORT`. It starts its own UI server and never reuses another run's server. Vite keeps a separate dependency cache for each `LEITWERK_BROWSER_ENGINE` so parallel engines cannot replace each other's optimized modules. The full gate assigns sibling output directories under `test-results/` to Chromium, WebKit, concurrent Firefox, and serial Firefox layout runs. No invocation may clean another invocation's traces or retry artifacts.
