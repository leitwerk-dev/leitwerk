...
- **Browser isolation:** Each Playwright invocation selects ephemeral API and UI ports, inherited by its workers through `LEITWERK_BROWSER_API_PORT` and `LEITWERK_BROWSER_UI_PORT`. It starts its own UI server and never reuses another run's server.
