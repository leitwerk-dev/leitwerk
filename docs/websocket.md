...
`GET /api/processes/:instanceId/turn-records/:turnRecordId/reasoning` returns `state: "live" | "committed"` and `throughEventSequence`. Live responses replay all recorded events for that exact turn record, without a lookback limit. Committed responses use the retained session tree plus recorded operational events. Ship the protocol, server, migrations, and bundled UI together; no new configuration is required.
